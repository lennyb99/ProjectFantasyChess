...
public class PlayModeCameraController : MonoBehaviour
{
    ...
    public void AdjustCameraPosition(List<GameObject> objects)
    {
        if (objects == null || objects.Count == 0)
        {
            Debug.LogWarning("The list of objects is empty or null. Cannot adjust camera position.");
            return;
        }
        ...
        Vector3 averagePosition = totalPosition / objects.Count;
        Camera.main.transform.position = new Vector3(averagePosition.x, averagePosition.y, Camera.main.transform.position.z);
    }
}
